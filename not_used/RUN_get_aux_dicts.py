...
ABOUT:
======
Given the initial params, do a fake pass through an epoch and output the
  auxilary dictionary

Uses the same config as the original train_pairHMM function

Will get an auxilary dictionary from both train_fn and eval_fn
...
def get_intermeds(args, dataloader_lst):
    ###########################################################################
    ...
    ### for given timepoint
    with open(f'{args.intermediates_folder}/TRAIN_FN_INTERMEDIATES.pkl','wb') as g:
        pickle.dump({'aux_dict': aux_dict,
                     't_array': t_array,
                     'params': pairHMM[-1].undo_param_transform(params),
                     'transf_params': params,
                     'hparams': hparams}, g)
    
    print('No gradient update applied')
    ...
    ### array; make sure code is calculating as Ian's code would, 
    ### for given timepoint
    with open(f'{args.intermediates_folder}/EVAL_FN_INTERMEDIATES.pkl','wb') as g:
        pickle.dump({'aux_dict': aux_dict,
                     't_array': t_array,
                     'params': pairHMM[-1].undo_param_transform(params),
                     'transf_params': params,
...
if __name__ == '__main__':
    ...
    with jax.disable_jit():
    	get_intermeds(args, dataloader_lst)
